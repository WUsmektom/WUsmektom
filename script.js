...
document.querySelectorAll('.variant-btn').forEach(button => {
    button.addEventListener('click', function() {
        ...
        const newImg = this.getAttribute('data-img'); // Ambil path gambar dari data-img
        const productImg = card.querySelector('.products-img img'); // Cari elemen gambar di dalam card
        
        if (newImg) {
            productImg.src = newImg; // Ubah sumber gambar
        }
        // ---------------------------------------
...
document.querySelectorAll('.btn-buy').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        const phoneNumber = "6288989797701"; 
        
        const message = `Halo Wedang Uwuh! Saya ingin memesan:%0A%0A` +
